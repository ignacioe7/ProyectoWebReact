      ...
      <div id="header" className="flex fixed z-50 w-full bg-gray-800 bg-gradient-to-b from-black/80 to-red-950/80 text-red-100 border-b border-red-50/30">
          ...
          <nav id="navbar" className="hidden md:flex">
            <ul className="flex my-4 mr-4 divide-x divide-red-500/40">
              {links.map((link, i) => <li key={`nav-item-${i}`} className="text-sm text-gray-400 hover:text-gray-500 px-3 hover:underline-offset-4 hover:underline"><a href={link.href}>{link.name}</a></li>)}
            </ul>
          </nav>
          <div className="inline-flex md:hidden ml-auto mr-8">
            <button
              id="mobile-menu-button"
              type="button"
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="-m-2.5 rounded-md p-2.5 text-stale-800"
            >
              <svg
                className="h-6 w-6"
                fill="none"
        ...
        <div
          id="mobile-menu"
          className={`${isMobileMenuOpen ? 'flex flex-col' : 'hidden'} w-full z-40 mt-20 bg-black/90 text-emerald-200 text-lg text-center gap-y-1 py-8`}
        >
          {links.map((link, i) => <a key={`mobile-nav-item-${i}`} href={link.href} className="hover:bg-black hover:text-emerald-100 py-3">{link.name}</a>)}
        </div>
      </div>
